...
function index()
	...
	entry({"admin", "system", "system"}, cbi("admin_system/system")) -- 08/15 bian rm, _("System"), 1
	...
	if nixio.fs.access("/bin/opkg") then
		entry({"admin", "system", "packages"}, call("action_packages")) -- 08/15 bian rm , _("Software"), 10
		entry({"admin", "system", "packages", "ipkg"}, form("admin_system/ipkg"))
	end

	entry({"admin", "system", "startup"}, form("admin_system/startup")) -- 08/15 bian rm:, _("Startup"), 45
	entry({"admin", "system", "crontab"}, form("admin_system/crontab")) -- 08/15 bian rm:, _("Scheduled Tasks"), 46

	if nixio.fs.access("/etc/config/fstab") then
		entry({"admin", "system", "fstab"}, cbi("admin_system/fstab")) -- 08/15 bian rm:, _("Mount Points"), 50
	...
	if nixio.fs.access("/sys/class/leds") then
		entry({"admin", "system", "leds"}, cbi("admin_system/leds")) -- 08/15 bian rm:), _("<abbr title=\"Light Emitting Diode\">LED</abbr> Configuration"), 60
	end
